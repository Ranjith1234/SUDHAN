...
if (location.pathname != "/index-rtl-fa.html"
...
function InitiateSideMenu() {
	...
	$("#sidebar-collapse")
			.on(
					...
					function() {
						...
						if ($(".sidebar-menu").closest("div").hasClass(
								"slimScrollDiv")) {
						...
						if (b) {
							...
							if ($('.page-sidebar').hasClass('sidebar-fixed')) {
								var position = (readCookie("rtl-support")
										|| location.pathname == "/index-rtl-fa.html" || location.pathname == "index-rtl-ar.html") ? 'right'
										: 'left';
								$('.sidebar-menu').slimscroll({
									height : 'auto',
									position : position,
	...
	$(".sidebar-menu").on('click', function(e) {
		...
		var href = $(menuLink).prop('href');
		var index = href.indexOf('#');
		
		if(index+1 != href.length){
...
function InitiateSettings() {
	...
	$('#checkbox_fixednavbar').change(
			function() {
				...
				if (($('#checkbox_fixedheader').is(":checked"))
						&& !($(this).is(":checked"))) {
	...
	$('#checkbox_fixedbreadcrumbs').change(
			function() {
...
function setCookiesForFixedSettings() {
	...
	createCookie("sidebar-fixed", $('#checkbox_fixedsidebar').is(':checked'),
			100);
	createCookie("breadcrumbs-fixed", $('#checkbox_fixedbreadcrumbs').is(
			':checked'), 100);
	createCookie("page-header-fixed",
			$('#checkbox_fixedheader').is(':checked'), 100);
...
// });
// var position = (readCookie("rtl-support") || location.pathname ==
// "/index-rtl-fa.html" || location.pathname == "index-rtl-ar.html") ? 'right' :
// 'left';
...
function removeClass(elem, cls) {
	...
	elem.className = str.replace(" " + cls, "").replace(/^\s+/g, "").replace(
			/\s+$/g, "");
}
